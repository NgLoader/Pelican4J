...
public class ListActionImpl<T> extends PteroActionImpl<List<T>> {

	protected final Function<JSONObject, T> handler;

	private ListActionImpl(P4J api, Route.CompiledRoute route, Function<JSONObject, T> handler) {
		super(api, route);
		this.handler = handler;
	}

	public static <T> ListActionImpl<T> onList(
			P4J api, Route.CompiledRoute route, Function<JSONObject, T> handler) {
		return new ListActionImpl<>(api, route, handler);
	}
